...

{
    ...
    /// 原始套接字监听
    /// </summary>
    public sealed class TmphRawSocketListener : TmphSocket
    {
        /// <summary>
        /// 默认获取的数据包的字节数(默认为以太网)
        /// </summary>
        ...
        /// </summary>
#if MONO
        private static readonly TmphMemoryPool buffers = TmphMemoryPool.GetPool(Laurent.Lee.CLB.Config.TmphPub.Default.RawSocketBufferSize);
#else
        ...
        public unsafe TmphRawSocketListener(Action<TmphSubArray<byte>> getPacket, IPAddress ipAddress, int packetSize = defaultBufferSize)
            : base(true)
        {
            if (getPacket == null) TmphLog.Error.Throw(TmphLog.TmphExceptionType.Null);
            if (packetSize <= 0 || packetSize > buffers.Size - sizeof(int)) TmphLog.Error.Throw(TmphLog.TmphExceptionType.IndexOutOfRange);
            try
            {
                ...
                Laurent.Lee.CLB.Threading.TmphThreadPool.TinyPool.Start(start, null, null);
                return;
            }
            catch (Exception error)
            {
                TmphLog.Error.Add(lastException = error, "监听初始化失败", false);
            }
            Dispose();
        }
        /// <summary>
        ...
        private unsafe void onReceive(object sender, SocketAsyncEventArgs async)
        {
            ...
            try
            {
                if (async.SocketError == SocketError.Success)
                {
                    ...
                    {
                        TmphSubArray<byte> packet = TmphSubArray<byte>.Unsafe(buffer, bufferIndex, count);
                        ...
                        if (bufferIndex <= maxBufferIndex || Interlocked.Exchange(ref buffer, null) == null)
                        {
                            ...
                            fixed (byte* bufferFixed = packet.Array) ++*(int*)bufferFixed;
                            bufferLock = 0;
                        }
                        else
                        {
        ...
        public static unsafe void Free(ref byte[] buffer)
        {
            if (buffer.length() == buffers.Size)
            {
                fixed (byte* bufferFixed = buffer)
                {
                    ...
                    try
                    {
                        if (--*(int*)bufferFixed == 0) buffers.Push(ref buffer);
                    }
...
#else
                TmphSocketAsyncEventArgsProxy.Push(ref async);
#endif
            }
