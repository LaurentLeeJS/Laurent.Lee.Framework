...
namespace Laurent.Lee.CLB.Sql.Cache.Whole
{
        ...
        where TCacheType : class
    {
        ...
        public TmphMember(Events.TmphCache<TValueType, TModelType> cache, Func<TModelType, TKeyType> getKey, Func<TKeyType, TTargetType> getValue, Expression<Func<TTargetType, TCacheType>> member)
        {
            if (cache == null || getKey == null || getValue == null || member == null) TmphLog.Error.Throw(TmphLog.TmphExceptionType.Null);
            TmphMemberExpression<TTargetType, TCacheType> expression = new TmphMemberExpression<TTargetType, TCacheType>(member);
