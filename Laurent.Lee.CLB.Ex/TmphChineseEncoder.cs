...
namespace Laurent.Lee.CLB
{
    ...
    /// 汉字编码检测
    /// </summary>
    public unsafe struct TmphChineseEncoder
    {
        /// <summary>
        /// UTF32文件BOM
        /// </summary>
        private static readonly uint utf32Bom = TmphFile.GetBom(Encoding.UTF32).Bom;

        /// <summary>
        /// UTF8文件BOM
        /// </summary>
        private static readonly uint utf8Bom = TmphFile.GetBom(Encoding.UTF8).Bom;

        /// <summary>
        /// Unicode文件BOM
        /// </summary>
        private static readonly uint unicodeBom = TmphFile.GetBom(Encoding.Unicode).Bom;

        /// <summary>
        /// 大端Unicode文件BOM
        /// </summary>
        private static readonly uint bigEndianUnicodeBom = TmphFile.GetBom(Encoding.BigEndianUnicode).Bom;
        ...
        private Encoding get(byte* dataFixed, int length)
        {
            ...
            Unsafe.TmphMemory.Fill((byte*)(gb18030Char = gb18030Chars), 0UL, 0x40 * sizeof(char) / sizeof(ulong));
            ...
            *end++ = (byte)endValue;
            ...
            if (utf16Count >= 0)
            {
                ...
                Unsafe.TmphMemory.Fill((byte*)(utf16Char = utf16Chars), 0UL, 0x10 * sizeof(char) / sizeof(ulong));
                ...
                Unsafe.TmphMemory.Fill((byte*)(bigUtf16Char = bigUtf16Chars), 0UL, 0x10 * sizeof(char) / sizeof(ulong));
        ...
        {
            *end = 0xff;
            uint TmphCode = *(uint*)start;
            while ((TmphCode & 0x80808080) == 0) TmphCode = *(uint*)(start += 4);
            if (start == end) return (endValue & 0x80) == 0;
            else if ((TmphCode & 0x80) == 0)
            {
                if (++start == end) return (endValue & 0x80) == 0;
                else if ((TmphCode & 0x8000) == 0)
                {
                    if (++start == end) return (endValue & 0x80) == 0;
                    else if ((TmphCode & 0x800000) == 0)
                    {
                        if (++start == end) return (endValue & 0x80) == 0;
                    }
                }
        ...
        private void ascii()
        {
            *end = 0x80;
            uint TmphCode = *(uint*)start;
            while ((TmphCode & 0x80808080) == 0) TmphCode = *(uint*)(start += 4);
            if (start != end && (TmphCode & 0x80) == 0
                && ++start != end && (TmphCode & 0x8000) == 0
                && ++start != end && (TmphCode & 0x800000) == 0)
            {
                ++start;
            }
        }
        ...
        private void gb2312()
        {
            ...
            {
                *end = 0;
                for (TmphCode = *(uint*)start; (TmphCode & 0x80808080) == 0x80808080; TmphCode = *(uint*)(start += 4))
                {
                    if ((byte)(TmphCode - 0xa1) <= (0xf7 - 0xa1) && (ushort)(TmphCode - 0xa100) < (0xfe00 - 0xa100 + 0x100))
                    {
                        if (gb18030Char[(ushort)TmphCode & 0x3f] == (ushort)TmphCode) ++gb2312Count;
                        else if ((byte)(TmphCode - (0xa0 + 10)) <= (15 - 10)
                            || ((byte)TmphCode == 0xd7 && (ushort)(TmphCode - 0xfad7) <= (0xfed7 - 0xfad7)))
                        {
                    ...
                    if ((byte)(TmphCode - 0xa1) <= (0xf7 - 0xa1) && (ushort)(TmphCode - 0xa100) < (0xfe00 - 0xa100 + 0x100))
                    {
                        if (gb18030Char[TmphCode & 0x3f] == TmphCode) ++gb2312Count;
                        else if ((byte)(TmphCode - (0xa0 + 10)) <= (15 - 10)
                            || ((byte)TmphCode == 0xd7 && (ushort)(TmphCode - 0xfad7) <= (0xfed7 - 0xfad7)))
                        {
                            gb2312Count = int.MinValue;
        ...
        private void gb18030()
        {
            ...
            while (start != end)
            {
                TmphCode = *(ushort*)start;
                if ((TmphCode & 0x80) != 0)
                {
                    if ((byte)(TmphCode - 0x81) <= (0xfe - 0x81) && (ushort)(TmphCode - 0x3000) < (0xfe00 - 0x3000 + 0x100))
                    {
                        if (TmphCode >= 0x4000)
                        {
                            if ((TmphCode & 0xff00) != 0x7f00 && ++start != end)
                            {
                                ++start;
                                if (gb18030Char[TmphCode & 0x3f] == TmphCode) ++gb18030Count;
                                continue;
                            }
                        }
                        else if (TmphCode < 0x3a00 && ++start != end && ++start != end)
                        {
                            TmphCode = *(ushort*)start;
                            if ((byte)(TmphCode - 0x81) <= (0xfe - 0x81)
                                && (ushort)(TmphCode - 0x3000) < (0x3900 - 0x3000 + 0x100) && ++start != end)
                            {
                                ++start;
                                continue;
                            }
                ...
                else if (++start != end && (TmphCode & 0x8000) == 0 && ++start != end)
                {
                    --end;
                    ascii();
                    if (start == end)
            ...
            while (start != end)
            {
                ...
                if ((TmphCode & 0x80) != 0)
                {
                    if (TmphCode >= 0xa100)
                    {
                        if (TmphCode <= 0xfe00)
                        {
                            if ((byte)TmphCode >= 0xb0 ? (byte)TmphCode <= 0xf7 : (byte)(TmphCode - 0x81) <= (0xa9 - 0x81))
                            {
                                if (++start != end)
                                {
                                    ++start;
                                    if (gb18030Char[TmphCode & 0x3f] == TmphCode) ++gbkCount;
                                    continue;
                                }
                            }
                        }
                    }
                    else if (TmphCode >= 0x4000 && (TmphCode & 0xff00) != 0x7f00)
                    {
                        if ((byte)TmphCode >= 0xa8 ? (byte)TmphCode <= 0xfe : (byte)(TmphCode - 0x81) <= (0xa0 - 0x81))
                        {
                            if (++start != end)
                            {
                                ++start;
        ...
        private void utf8()
        {
            ...
            Unsafe.TmphMemory.Fill((byte*)utf8Char, 0UL, 0x10 >> 1);
            ...
            {
                *end = 0;
                for (TmphCode = *(uint*)start; (TmphCode & 0x80808080) == 0x80808080;)
                {
                    code1 = TmphCode;
                    if ((TmphCode &= 0xe0) >= 0xe0)
                    {
                        if (TmphCode == 0xe0)
                        {
                            if ((code1 & 0xc0c000) == 0x808000)
                            {
                                TmphCode = *(uint*)(start += 3);
                        ...
                        {
                            TmphCode = *(uint*)(start += 4);
                            continue;
                        }
                    }
                    else if (TmphCode == 0xc0 && (code1 & 0xc000) == 0x8000)
                    {
        ...
        private void big5()
        {
            ...
            Unsafe.TmphMemory.Fill((byte*)big5Char, 0UL, 0x40 * sizeof(char) / sizeof(ulong));
            ...
            while (start != end)
            {
                ...
                {
                    if ((byte)(TmphCode - 0x81) <= (0xfe - 0x81)
                        && (ushort)(TmphCode - 0x4000) < (0xfe00 - 0x4000 + 0x100) && (TmphCode & 0xff00) != 0x7f00)
                    {
                        if ((byte)TmphCode >= 0xa4)
                        {
                            if ((byte)TmphCode <= 0xc6)
                            {
                                if (((byte)TmphCode != 0xc6 || (TmphCode & 0x8000) == 0) && ++start != end)
                                {
                                    ++start;
                                    if (big5Char[(TmphCode >> 8) & 0x3f] == TmphCode) ++big5Count;
                                    continue;
                                }
                            }
                            else if ((byte)(TmphCode - 0xc9) <= (0xf9 - 0xc9) && ++start != end)
                            {
                                ++start;
                                continue;
                            }
                        }
                        else if ((byte)TmphCode >= 0xa1)
                        {
                            if (((byte)TmphCode != 0xa3 || (TmphCode & 0xc000) == 0) && ++start != end)
                            {
                                ++start;
                                if (big5Char[(TmphCode >> 8) & 0x3f] == TmphCode) ++big5Count;
                                continue;
        ...
        private void utf16()
        {
            ...
            {
                uint TmphCode = *(uint*)start;
                if (utf16Char[TmphCode & 0xf] == (char)TmphCode) ++utf16Count;
                else if (bigUtf16Char[(TmphCode >> 8) & 0xf] == (char)TmphCode) ++bigUtf16Count;
                TmphCode >>= 16;
                if (utf16Char[TmphCode & 0xf] == TmphCode) ++utf16Count;
                else if (bigUtf16Char[(TmphCode >> 8) & 0xf] == TmphCode) ++bigUtf16Count;
                start += 4;
            }
        }
        ...
        private void utf32()
        {
            ...
            Unsafe.TmphMemory.Fill((byte*)utf32Char, 0UL, 0x10 >> 1);
            ...
            //Laurent.Lee.CLB.sys.memory.set((byte*)bigUtf32Char, 0U, 0x10);
            ...
            {
                uint TmphCode = *(uint*)start;
                if (utf32Char[TmphCode & 0xf] == TmphCode) ++utf32Count;
                //else if (bigUtf32Char[(TmphCode >> 24) & 0xf] == TmphCode) ++bigUtf32;
                start += 4;
            }
        }
        ...
        {
            return data != null ? (data.Length != 0 ? new TmphChineseEncoder().Get(data) : Encoding.ASCII) : null;
        }
        ...
        /// <param name="length">字节数量</param>
        /// <returns>编码,失败为null</returns>
        public static Encoding ChineseEncoding(byte* data, int length)
        {
            return new TmphChineseEncoder().get(data, length);
        }
