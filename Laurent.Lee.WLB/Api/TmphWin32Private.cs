...
namespace Laurent.Lee.WLB
{
    ...
    public static partial class TmphWin32
    {
        ...
        /// 执行获取当前运行的操作系统版本。
        /// </summary>
        /// <returns><see cref="TmphPlatform"/> 的值之一，他表示当前运行的操作系统版本。</returns>
        private static TmphPlatform GetCurrentPlatform()
        {
            ...
            switch (os.Platform)
            {
                case (PlatformID.Win32Windows): // Win95, Win98 or Me
                    ...
                    {
                        case (0): // 95
                            pt = TmphPlatform.Windows95;
                            break;

                        case (10): // 98
                            if (os.Version.Revision.ToString() == "2222A")
                                pt = TmphPlatform.Windows982ndEdition;
                            else
                                pt = TmphPlatform.Windows98;
                            break;

                        case (90): // winme
                            pt = TmphPlatform.WindowsME;
                            break;

                        default: // Unknown
                            pt = TmphPlatform.UnKnown;
                            break;
                    }
                    break;
                ...
                case (PlatformID.Win32NT): //Win2k or Xp or 2003
                    ...
                    {
                        case (3):
                            pt = TmphPlatform.WindowsNT351;
                            break;

                        case (4):
                            pt = TmphPlatform.WindowsNT40;
                            break;

                        case (5):
                            if (os.Version.Minor == 0)
                                pt = TmphPlatform.Windows2000;
                            else if (os.Version.Minor == 1)
                                pt = TmphPlatform.WindowsXP;
                            else if (os.Version.Minor == 2)
                                pt = TmphPlatform.Windows2003;
                            else
                                pt = TmphPlatform.UnKnown;
                            break;

                        case (6):
                            pt = TmphPlatform.WindowsVista;
                            break;
        ...
        /// 表示操作系统平台。
        /// </summary>
        private enum TmphPlatform : byte
        {
            /// <summary>
            /// Windows 95 操作系统.
            /// </summary>
        ...
        private enum TmphDriverError : byte
        {
            /// <summary>
            /// 设备无错误。
            /// </summary>
        ...
        /// 根据指定的设备信息生成设备的详细信息。
        /// </summary>
        /// <param name="phdinfo">一个 <see cref="TmphIdSector"/></param>
        /// <returns></returns>
        private static TmphHDiskInfo GetHardDiskInfo(TmphIdSector phdinfo)
        {
            TmphHDiskInfo hdd = new TmphHDiskInfo();
        ...
        /// <returns>非零表示成功，零表示失败。</returns>
        [DllImport("kernel32.dll", SetLastError = true)]
        private static extern int DeviceIoControl(IntPtr hDevice, uint dwIoControlCode, ref TmphSendCmdInParams lpInBuffer,
                                                  uint nInBufferSize, ref TmphSendCmdOutParams lpOutBuffer,
                                                  uint nOutBufferSize, ref uint lpBytesReturned,
                                                  [Out] IntPtr lpOverlapped);
        ...
        private struct TmphGetVersionOutParams
        {
            /// <summary>
            /// IDE设备的二进制硬件版本。
            /// </summary>
        ...
        [StructLayout(LayoutKind.Sequential, Pack = 1)]
        private struct TmphSendCmdInParams
        {
            /// <summary>
            /// 输出的数据缓冲大小。
            /// </summary>
            ...
            /// 保存向系统发送的磁盘设备命令的数据结构。
            /// </summary>
            public TmphIdeRegs irDriveRegs;
        ...
        /// </remarks>
        [StructLayout(LayoutKind.Sequential, Pack = 1)]
        private struct TmphDriverStatus
        {
            /// <summary>
        ...
        /// </summary>
        [StructLayout(LayoutKind.Sequential, Pack = 1)]
        private struct TmphSendCmdOutParams
        {
            /// <summary>
            /// 表示所有二进制信息的缓存大小。
            /// </summary>
            ...
            /// 表示查询到设备的错误信息状态。
            /// </summary>
            public TmphDriverStatus DriverStatus;

            /// <summary>
            /// 表示系统返回的设备硬件信息的二进制数据结构。
            /// </summary>
            public TmphIdSector bBuffer;
        }
        ...
        public struct TmphRECT
        {
            ...
            public TmphRECT(int left, int top, int right, int bottom)
            {
                this.left = left;
                this.top = top;
                this.right = right;
            ...
            /// <returns></returns>
            public static TmphRECT FromXYWH(int x, int y, int width, int height)
            {
                return new TmphRECT(x, y, x + width, y + height);
            }
        ...
        /// </summary>
        [StructLayout(LayoutKind.Sequential)]
        public sealed class TmphSCROLLINFO
        {
            public TmphSCROLLINFO()
            {
                this.cbSize = Marshal.SizeOf(typeof(TmphSCROLLINFO));
            }

            public TmphSCROLLINFO(int mask, int min, int max, int page, int pos)
            {
                this.cbSize = Marshal.SizeOf(typeof(TmphSCROLLINFO));
                this.fMask = mask;
                this.nMin = min;
        ...
        private static readonly TmphPlatform currentOs;
