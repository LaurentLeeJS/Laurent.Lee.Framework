...
namespace Maticsoft.DBUtility
{
    ...
    /// 数据访问类，可用于访问不同数据库
    /// </summary>
    public class TmphDbHelperSQLP
    {
        //数据库连接字符串(web.config来配置)，可以动态更改connectionString支持多数据库.
        public string connectionString = "连接字符串";

        public TmphDbHelperSQLP()
        {
        }

        public TmphDbHelperSQLP(string ConnectionString)
        {
        ...
        public int ExecuteSqlTran(List<CommandInfo> list, List<CommandInfo> oracleCmdSqlList)
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                try
                {
                    ...
                    bool res = TmphOracleHelper.ExecuteSqlTran(oraConnectionString, oracleCmdSqlList);
                    if (!res)
                    {
                        tx.Rollback();
