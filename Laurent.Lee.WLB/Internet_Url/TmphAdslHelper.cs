...
{
    public class TmphRas
    {
        // Fields
        private bool bConnected;

        private TmphConnectionNotify ConnectNotify;
        ...
        public TmphRas(TmphConnectionNotify ConnectionDelegate, double interval)
        {
            ...
            this.Rasconn = new TmphRASCONN[1];
        ...
        {
            TmphRASENTRYNAME[] lprasentryname = new TmphRASENTRYNAME[1];
            ...
            switch (nErrorValue)
            {
                ...
                case 0x25b:
                    lprasentryname = new TmphRASENTRYNAME[lpcEntries];
                    lprasentryname[0].dwSize = Marshal.SizeOf(lprasentryname[0]);
                    break;
        ...
        public bool HangUp(out string strError)
        {
            ...
            foreach (TmphRASCONN rasconn in this.Rasconn)
            {
        ...
        private void RasDialFunc(uint unMsg, TmphRASCONNSTATE rasconnstate, int dwError)
        {
            if (dwError != 0)
            {
            ...
            {
                if (rasconnstate == TmphRASCONNSTATE.RASCS_PortOpened)
                {
                    this.ConnectNotify("端口已经打开.", 1);
                }
                if (rasconnstate == TmphRASCONNSTATE.RASCS_ConnectDevice)
                {
                    this.ConnectNotify("正在拨...", 1);
                }
                if (rasconnstate == TmphRASCONNSTATE.RASCS_Authenticate)
                {
                    this.ConnectNotify("正在验证用户名与密码.", 1);
                }
                if (rasconnstate == TmphRASCONNSTATE.RASCS_Connected)
                {
        ...
        [DllImport("rasapi32.dll", CharSet = CharSet.Auto)]
        private static extern int RasEnumConnections([In, Out] TmphRASCONN[] lprasconn, ref int lpcb, ref int lpcConnections);

        [DllImport("rasapi32.dll", CharSet = CharSet.Auto)]
        private static extern int RasEnumEntries(string reserved, string lpszPhonebook, [In, Out] TmphRASENTRYNAME[] lprasentryname, ref int lpcb, ref int lpcEntries);

        [DllImport("rasapi32.dll", CharSet = CharSet.Auto)]
        private static extern int RasGetConnectStatus(int hrasconn, ref TmphRASCONNSTATUS lprasconnstatus);

        [DllImport("rasapi32.dll", CharSet = CharSet.Auto)]
        private static extern int RasGetEntryDialParams(string lpszPhonebook, ref TmphRASDIALPARAMS lprasdialparams, ref bool lpfPassword);
        ...
        public delegate void TmphConnectionNotify(string strNotify, int bConnect);

        [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Auto)]
        private struct TmphRASCONN
        {
            internal int dwSize;
        ...
        [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Auto)]
        private struct TmphRASCONNSTATUS
        {
            internal int dwSize;
            internal TmphRas.TmphRASCONNSTATE rasconnstate;
        ...
        private delegate void TmphRasDialEvent(uint unMsg, TmphRas.TmphRASCONNSTATE rasconnstate, int dwError);

        [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Auto)]
        private struct TmphRASDIALPARAMS
        {
            internal int dwSize;
        ...
        [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Auto)]
        private struct TmphRASENTRYNAME
        {
            internal int dwSize;
